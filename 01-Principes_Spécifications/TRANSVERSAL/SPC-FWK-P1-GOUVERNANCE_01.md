...
revision: 1
titre: 00 GouvernanceCadreSpécifications-ExigencesDKG
titre_court: GOUV
phase_code: P1
...
- **Validation aux Frontières (Boundary Validation) [`EXG-OR-07`]** : Toute donnée structurée ou semi-structurée entrant dans l'application (configurations, requêtes API, flux CTI bruts, entités candidates d'interception) doit être validée et typée par un modèle Pydantic V2 (`BaseModel` ou `BaseSettings`) avant d'être convertie en triplets RDF/RDFLib ou consommée par les moteurs d'inférence.
