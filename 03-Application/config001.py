...
Phase 8 Active : SOC Orchestrator, Partitionnement et Filtrage Frugal.
"""
...
class DKGConfig(BaseSettings):
    ...
    # Répertoires Snapshots & Masters (Phases 1 à 7)
    @property
    ...
    # --- NOUVEAUTÉS PHASE 8 : SOC Orchestrator & Partitionnement Frugal ---
    @property
    def dir_snapshot_p8(self) -> Path:
        return self.dir_data / "Snapshots_Phases" / "Phase8_SOC_Orchestrator"

    @property
    def dir_inputs_p8(self) -> Path:
        return self.dir_data / "Input_Phases" / "Phase8_SOC_Orchestrator"
    ...
    @property
    def dir_delta_buffer(self) -> Path:
        return self.dir_data / "Delta_Buffer"

    # --------------------------------------------------------------------------
    # 2. SEUILS & PARAMETRES IA & GREEN DEV
    # --------------------------------------------------------------------------
    ...
    # Contraintes Green Dev (Phase 8)
    max_triples_in_memory: int = Field(default=50000)
    enable_incremental_loading: bool = Field(default=True)
    require_hitm_for_tbox: bool = Field(default=True)
    ...
    def ensure_directories_exist(self) -> None:
        ...
        target_dirs = [
            ...
            self.dir_delta_buffer, self.dir_private_red
        ]
...
DIR_DELTA_BUFFER = _settings.dir_delta_buffer
...
# Chemins Phase 8 (Buffers et Deltas Incrémentaux)
INPUT_PHASE8_REGULATION_PATH = DIR_INPUTS_P8 / "gdpr_article32_requirements.json"
DELTA_BUFFER_PATH = DIR_DELTA_BUFFER / "active_delta_buffer.ttl"
AUDIT_GOVERNANCE_REPORT_PATH = DIR_SNAPSHOT_P8 / "audit_governance_report.json"
...
MAX_TRIPLES_IN_MEMORY = _settings.max_triples_in_memory
ENABLE_INCREMENTAL_LOADING = _settings.enable_incremental_loading
REQUIRE_HITM_FOR_TBOX = _settings.require_hitm_for_tbox
